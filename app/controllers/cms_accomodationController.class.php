<?php
class Cms_accomodationController extends Controller{
	...
	public function index(){
		...
		//Set page
		parent::set('active', 'accomodation');
	}
	...
	public function submit($params){
		...
		if(isset($params['step']) && !empty($params['step']))
			switch(strtolower($params['step'])){
				...
				break;
				default: parent::redirect('cms'.DS.'accomodation', 'error');
			}
			...
			if(isset($params['count']) && $params['count'] > 0){
				for($i=1; $i<=$params['count']; $i++){
					...
					if(isset($params['file-' . $i]) && $params['file-' . $i]['error'] == 0){
						...
						parent::uploadAndResizeImage(150, 205, $params['file-' . $i], $id.'-'.$tmpId.'-'.$params['file-' . $i]['name'], 'accomodation', true);
					}
				}
			}
			
			parent::redirectCustom('cms'.DS.'accomodation'.DS.$params['action'], array('id' => $id, 'step' => 'second'));
		}else parent::redirect('cms'.DS.'accomodation'.DS.$params['action'], 'error');
	}
	
	private function submitSecond($params){

		if($id = $this->db->submit($params))
			parent::redirectCustom('cms'.DS.'accomodation'.DS.$params['action'], array('id' => $id, 'step' => 'third'));
		else 
			parent::redirect('cms'.DS.'accomodation'.DS.$params['action'], 'error');
	}

	private function submitThird($params){

		if($this->db->submit($params))
			parent::redirect('cms'.DS.'accomodation', 'success');
		else 
			parent::redirect('cms'.DS.'accomodation'.DS.$params['action'], 'error');
	}
	...
	public function deleteImage($params){
		...
		if($this->db->deleteImage($params)) parent::redirect('cms'.DS.'accomodation'.DS.$params['id'].DS.'edit', 'success');
		else parent::redirect('cms'.DS.'accomodation', 'error');
	}
	...
	public function deleteService($params){
		...
		if($this->db->deleteService($params)) parent::redirectCustom('cms'.DS.'accomodation'.DS.'edit', array('id' => $params['id'], 'step' => 'second'));
		else parent::redirect('cms'.DS.'accomodation', 'error');
	}
	...
	public function delete($params){
		...
		if($this->db->delete($params)) parent::redirect('cms'.DS.'accomodation', 'success');
		else parent::redirect('cms'.DS.'accomodation', 'error');
	}
}