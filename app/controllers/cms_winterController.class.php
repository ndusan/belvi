<?php
class Cms_winterController extends Controller{
	...
	public function submit($params){
		...
		if(isset($params['step']) && !empty($params['step']))
			switch(strtolower($params['step'])){
				...
				break;
				default: parent::redirect('cms'.DS.'winter', 'error');
			}
	...
	private function submitFirst($params){
		...
		if($id = $this->db->submit($params)){
			...
			parent::redirectCustom('cms'.DS.'winter'.DS.$params['action'], array('id' => $id, 'step' => 'second'));
		}else parent::redirect('cms'.DS.'winter'.DS.$params['action'], 'error');
	}
	
	private function submitSecond($params){

		if($id = $this->db->submit($params))
			parent::redirectCustom('cms'.DS.'winter'.DS.$params['action'], array('id' => $id, 'step' => 'third'));
		else 
			parent::redirect('cms'.DS.'winter'.DS.$params['action'], 'error');
	}

	private function submitThird($params){

		if($this->db->submit($params))
			parent::redirect('cms'.DS.'winter', 'success');
		else 
			parent::redirect('cms'.DS.'winter'.DS.$params['action'], 'error');
	}
	...
	public function deleteImage($params){
		...
		if($this->db->deleteImage($params)) parent::redirect('cms'.DS.'winter'.DS.$params['id'].DS.'edit', 'success');
		else parent::redirect('cms'.DS.'winter', 'error');
	}
	...
	public function deleteService($params){
		...
		if($this->db->deleteService($params)) parent::redirectCustom('cms'.DS.'winter'.DS.'edit', array('id' => $params['id'], 'step' => 'second'));
		else parent::redirect('cms'.DS.'winter', 'error');
	}
	...
	public function delete($params){
		...
		if($this->db->delete($params)) parent::redirect('cms'.DS.'winter', 'success');
		else parent::redirect('cms'.DS.'winter', 'error');
	}
}