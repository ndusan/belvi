...
class Cms_homeModel extends Model{
	...
	public function setVisible($params){
 		...
 		$query = sprintf("UPDATE `accomodation_types` SET `visible`='%s' WHERE `id`='%s'",
	...
	public function setPosition($params){
		...
		if(isset($params['dir']) && !empty($params['dir'])){
			switch($params['dir']){
				case 'up':
					$query_current = sprintf("SELECT * FROM `accomodation_types` WHERE `id`='%s'",
											mysql_real_escape_string($params['id'])
											);
					...
					//Switch places
					$query_switch1 = sprintf("UPDATE `accomodation_types` SET `position`='%s' WHERE `id`='%s'",
											mysql_real_escape_string($row_current['position']),
											mysql_real_escape_string($row_next['id'])
											);
					mysql_query($query_switch1);
					
					$query_switch2 = sprintf("UPDATE `accomodation_types` SET `position`='%s' WHERE `id`='%s'",
	...
	public function getLinks(){
		...
		$query = sprintf("SELECT * FROM `accomodation_types`");
		return parent::query($query);
	}
	...
	public function submit($params){
		...
		if(isset($params['id']) && !empty($params['id'])){
			...
			$query = sprintf("UPDATE `carousel` SET `name`='%s', `accomodation_type_id`='%s' WHERE `id`='%s'",
			...
			if(isset($params['file']) && $params['file']['error'] == 0){
				...
				$query_img = sprintf("UPDATE `carousel` SET `image`='%s' WHERE `id`='%s'",
			...
			$position = 1;
			$query_position = sprintf("SELECT `position` FROM `carousel` ORDER BY `position` DESC LIMIT 0, 1");
			...
			$query = sprintf("INSERT INTO `carousel` SET `name`='%s', `accomodation_type_id`='%s', `image`='%s', `position`='%s'",
							mysql_real_escape_string($params['name']),
							mysql_real_escape_string($params['link']),
	...
	public function getCarousels(){
		...
		$query = sprintf("SELECT `carousel`.*, `accomodation_types`.`name` AS `accomodation_type` 
						FROM `carousel`
						INNER JOIN `accomodation_types` ON `accomodation_types`.`id`=`carousel`.`accomodation_type_id`
	...
	public function deleteImage($params){
 		...
 		$query = sprintf("UPDATE `carousel` SET `image`='' WHERE `id`='%s'",
 						mysql_real_escape_string($params['id'])
 						);
 		mysql_query($query);
 		return true;
	...
	public function setPositionCarousel($params){
		
		if(isset($params['dir']) && !empty($params['dir'])){
			switch($params['dir']){
				case 'up':
					$query_current = sprintf("SELECT * FROM `carousel` WHERE `id`='%s'",
											mysql_real_escape_string($params['id'])
											);
					$res_current = mysql_query($query_current);
					$row_current = mysql_fetch_assoc($res_current);
					$query_next = sprintf("SELECT * FROM `carousel` WHERE 	`id`!='%s' AND 
																				`position`>'%s'
																				ORDER BY `position` ASC
																				LIMIT 0, 1",
											mysql_real_escape_string($row_current['id']),
					...
					$row_next = mysql_fetch_assoc($res_next);
					//Switch places
					$query_switch1 = sprintf("UPDATE `carousel` SET `position`='%s' WHERE `id`='%s'",
											mysql_real_escape_string($row_current['position']),
											mysql_real_escape_string($row_next['id'])
											);
					mysql_query($query_switch1);
					
					$query_switch2 = sprintf("UPDATE `carousel` SET `position`='%s' WHERE `id`='%s'",
											mysql_real_escape_string($row_next['position']),
											mysql_real_escape_string($row_current['id'])
											);
					mysql_query($query_switch2);
				...
				case 'down':
					...
					$query_prev = sprintf("SELECT * FROM `carousel` WHERE 	`id`!='%s' AND 
																				`position`<'%s' 
																				ORDER BY `position` DESC
																				LIMIT 0, 1",
											mysql_real_escape_string($row_current['id']),
