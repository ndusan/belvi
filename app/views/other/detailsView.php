<div class="details_content">
	<div class="offer_page_title_bg">
		<div class="page_title_text cufon"><?php echo $accType['acc_type']['name']." &raquo; ". $accom['acc'][0]['name']; ?></div>
	</div>
	<div class="details_page_carusel">
		<?php include_once VIEW_PATH.'home'.DS.'_customCarousel.php';?>	
	</div>
	<div class="fb"><iframe src="http://www.facebook.com/plugins/like.php?href=<?php echo "http://". $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']; ?>&amp;layout=standard&amp;show_faces=false&amp;width=430&amp;action=like&amp;font=arial&amp;colorscheme=light&amp;height=35" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:430px; height:35px;" allowTransparency="true"></iframe></div>
	<div style="clear:both"></div>
	
	<div class="currPart">
	<?php echo $accType['acc_type']['desc']; ?>
	</div>
	
	<div class="tabelar">
		<?php if(isset($accType) && !empty($accType)):?>
		<table cellspacing="1px" width="100%">
			<thead>
              	<tr>
				    <th scope="col" width="170px">TIP SMEŠTAJA</th>
				    <th scope="col">Kat.</th>
				    <th scope="col">USLUGA</th>
				    <?php foreach($accType['dates'] as $d):?>
				    <th scope="col">
						<?php
							$array = explode("-", $d['date']);
							echo strtoupper($html->toSerbian(date("M", mktime(0, 0, 0, $array[1], $array[0], $array[2])))) . "<br/>" .$array[0];
						?>
					</th>
				   	<?php endforeach;?>
				</tr>
				<tr>
				  	<th colspan="3" scope="col">broj noćenja &raquo;</th>
				  	<?php foreach($accType['dates'] as $d):?>
				    <th scope="col"><?php echo $d['nights']; ?></th>
					<?php endforeach;?>
  				</tr>
  				<?php if(isset($accType['prices']) && !empty($accType['prices'])):?>
  				<?php foreach($accType['prices'] as $p):?> 
  				<tr>
				    <td colspan="3" scope="col"><?php echo $p['transport'];?></td>
				    <td scope="col" colspan="<?php echo count($accType['dates']);?>"><?php echo $p['price']; ?></td>
				 </tr>
				 <?php endforeach;?>
				 <?php endif; ?>
            </thead>
            <?php if(isset($accom) && !empty($accom)):?>
            <tbody>
            	<?php $basicName = "";
            		  $firstLetter = "";
            		  foreach($accom['acc'] as $ac):
            		  	if($basicName != $ac['type']):
            		  		$basicName = $ac['type'];
            		  		$firstLetter = substr($ac['type'], 0, 1);
            	?>
            	<tr>
    				<th colspan="<?php echo 3 + count($accType['dates']); ?>" scope="col"><?php echo strtoupper($ac['type']); ?></th>
 			   </tr>
 			   <?php 
 			   		 endif; 
 			   		 
 			   		 if(isset($ac['service']) && !empty($ac['service'])):?>
 			   <?php $first = true; 
 			   		 $cnt =0;
 			   ?>
 			   <?php foreach(($ac['service']) as $a):?>
			   <tr class="<?php echo (++$cnt%2==0?'lite':'dark');?>">
			   	<?php if($first): $first = false; ?>
			    <td rowspan="<?php echo count($ac['service']); ?>" scope="row">
		    		<?php echo strtoupper($ac['name']); ?><br />
		    		<?php echo $ac['add']; ?>
			    </td>
			    
			    <td rowspan="<?php echo count($ac['service']); ?>" scope="row"><?php echo $ac['cat']; ?></td>
			    <?php endif;?>
			    <td class="al"><strong><?php echo $a['service']; ?></strong></td>
			    <?php foreach($a['price'] as $price):?>
			    <td><?php echo $price['price']; ?></td>
			    <?php endforeach;?>
		      </tr>
		      <?php endforeach; ?>
		      <?php endif; ?>

		      <?php endforeach; ?>
            </tbody>
            <?php endif; ?>
		</table>
		<?php else: ?>
		<div class="noResults" style="width: 98%;">
			Trenutno ne postoji cenovnik za izabrani city break
		</div>
		<?php endif;?>
	</div>
	<?php if(isset($accType['acc_type']['desc_bottom']) && !empty($accType['acc_type']['desc_bottom'])):?>
	<div class="currPart_bottom">
	<?php echo $accType['acc_type']['desc_bottom']; ?>
	</div>
	<?php endif; ?>
	
	
	
	<div class="boxHolder">
		<div class="details_tabs">
			 <ul>
	            <li><a href="javascript:;" class="tab active" id="link-desc-acc" rel="<?php echo $params['detail']; ?>">Program putovanja i uslovi</a></li>
	            <li><a href="javascript:;" class="tab" id="link-conditions-acc" rel="<?php echo $params['detail']; ?>">Opis</a></li>
	            <li><a href="javascript:;" class="tab" id="link-map-acc" rel="<?php echo $params['detail']; ?>">Mapa</a></li>
	            <li><a href="javascript:;" class="tab" id="link-weather-acc" rel="<?php echo $params['detail']; ?>">Vremenska prognoza</a></li>
	        </ul>
		</div>
		<div class="tabContent"><!-- load content -->
		</div>
	</div>
	
	<script type="text/javascript">
        $(document).ready(function(){
        	//Load by default
			$(".tabContent").addClass('loader');
			$.get(	'<?php echo BASE_PATH.'ajax_get_tab_page'; ?>',
					{'tab'	:	'link-desc-acc',
					 'name'	:	'<?php echo $params['detail']; ?>'
					},
					function(data){
						if(data){
							$(".tabContent").removeClass('loader').html(data);
						}
					},
					"html"
			);
            
			$(".tab").each(function(){
				$(this).click(function(){
					var id = $(this).attr('id');
					//Hide all tabs
					$(".tab").each(function(){
						if($(this).attr('id') != id) $(this).removeClass('active');
						else{
							$(this).addClass('active');
							$(".tabContent").addClass('loader').html('');
							//Ajax call for page
							$.get(	'<?php echo BASE_PATH.'ajax_get_tab_page'; ?>',
									{'tab'	:	$(this).attr('id'),
									 'name'	:	$(this).attr('rel')
									},
									function(data){
										if(data){
											$(".tabContent").removeClass('loader').html(data);
										}
									},
									"html"
							);
						}
					});
				});
			});
        });
	</script>
</div>

